...
function sacp_init() {
    ...
    if ( function_exists('shopp') ) {
		...
		$exclude = array('ShoppOfflinePayment' => 'ShoppOfflinePayment');
		...
		if ( ! empty($active) ) {
			...
			foreach ( $gateways as $key => $value ) {
				...
				$gateway = preg_split('/-/', $value);
				// if gateway name is not present in $exclude, apply the auto-capture filter
				if ( ! isset($exclude[ $gateway[0] ] ) )
					add_filter('shopp_purchase_order_' . strtolower($gateway[0]) . '_processing', create_function('', 'return "sale";'));
			}
		}
    }
}